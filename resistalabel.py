...
# Made a few changes to pull values and settings from a json to make it a bit easier 
...
def main() -> None:
    ...
    try:
        ...
        print("Error: 'resistor_values.json' not found. Please ensure the file exists.")
        return
    ...
    # Render the stickers for each set of values and tolerances
    for i, values in enumerate(resistor_values):
        tolerance_value = tolerance_values[i] if i < len(tolerance_values) else None
        render_stickers(c, layout, values, tolerance_value,
